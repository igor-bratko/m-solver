...
public class ApicoClient {
    ...
    public Mono<List<String>> initRequest(String magic) {
        return client.get().uri("/" + magic)
                .retrieve()
                .bodyToMono(new ParameterizedTypeReference<>() {});
    }

    public Mono<SvgPart> postMagic(int node, String magic, String payload, Map<Integer, String> pow) {
    ...
    private static RetryBackoffSpec staticRetry() {
        return Retry.fixedDelay(10, ofMillis(50))
                .filter(ex -> ex.getLocalizedMessage().contains("too early or too late")
                              || ex.getLocalizedMessage().contains("currently unavailable"));
    }
        ...
        return Retry.from(companion -> companion
                .flatMap(rs -> {
                            ...
                            if (msg.contains("too early or too late")) {
                                try {
                                    ParsedToken.Params params = om.readValue(msg, ErrorDto.class).issues().get(0).params();
